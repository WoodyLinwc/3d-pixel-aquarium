...
export const Fish: React.FC<FishProps> = ({ 
  ...
  // Movement state
  const [direction, setDirection] = useState(new THREE.Vector3(Math.random() > 0.5 ? 1 : -1, 0, (Math.random() - 0.5) * 0.4));
  ...
  useFrame((state) => {
    ...
    fish.position.y += Math.sin(time * verticalFrequency) * verticalAmplitude * 0.1;
  ...
  return (
    <mesh ref={meshRef} position={position} scale={[scale, scale, 1]} castShadow>
